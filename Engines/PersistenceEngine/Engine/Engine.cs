...
namespace Charon.Engines.PersistenceEngine
{
    public class Engine
    {
        ...
        public void Start()
        {
            _logger.Log(LogLevel.Info, message: "Initialising PersistenceEngine");
            ...
            _logger.Log(LogLevel.Info, "Connection to RabbitMQ instance @ " + config.RabbitMQAddress);
            _busManager = new BusManager("persistenceengine", "Tastypersistence1", config.RabbitMQAddress, config.QueueDataTearDown);
        ...
        public void Stop()
        {
            _logger.Log(LogLevel.Info, "PersistenceEngine is shutting down");
            _busManager.Dispose();
        }
    }
}
